/**
 * Toonflow AI供应商 - Z-Image 文生图（本地 ComfyUI）
 * @version 1.0
 */
...
const vendor: VendorConfig = {
  ...
  version: "1.0",
  author: "Toonflow",
  name: "Z-Image T2I（本地 ComfyUI）",
  description: "基于本地 ComfyUI 的 Z-Image Turbo 文生图工作流。需要 ComfyUI 环境已配置 Z-Image Turbo 模型。",
  inputs: [
...
const imageRequest = async (config: ImageConfig, model: ImageModel): Promise<string> => {
  ...
  logger(`[Z-Image T2I] 开始生成图片，提示词长度: ${config.prompt.length}`);
  ...
  // 3. 设置分辨率
  let width = 1360, height = 768;
  if (config.size === "1K") {
    width = config.aspectRatio === "16:9" ? 1024 : 576;
    height = config.aspectRatio === "16:9" ? 576 : 1024;
  } else if (config.size === "2K") {
    width = config.aspectRatio === "16:9" ? 1360 : 768;
    height = config.aspectRatio === "16:9" ? 768 : 1360;
  ...
  logger(`[Z-Image T2I] 分辨率: ${width}x${height}`);
  ...
  logger(`[Z-Image T2I] 任务已提交，ID: ${promptId}`);
  ...
  if (result.error) throw new Error(`Z-Image T2I 生成失败: ${result.error}`);
  ...
  logger(`[Z-Image T2I] 下载图片: ${downloadUrl}`);
