...
const vendor: VendorConfig = {
  ...
  models: [
    ...
    },
    {
      ...
      modelName: "flux2-klein-t2i",
      type: "image",
      mode: ["multiReference"],
      associationSkills: "FLUX.2 Klein 9B T2I 多参考图编辑，支持最多3张参考图进行图像编辑与合成",
    },
    {
...
const WORKFLOW_FLUX2_KLEIN_T2I = {
  "1": {
    "inputs": {
      "model_name": "flux-2-klein-9b-fp8.safetensors",
      "weight_dtype": "default",
...
/** 生成随机 LoRA 强度 (0.5 - 0.7) */
const generateLoraStrength = (): number => Math.round((Math.random() * 0.2 + 0.5) * 10) / 10;
...
/**
 * FLUX.2 Klein T2I（WORKFLOW_FLUX2_KLEIN_T2I）
 * 来源：C:\aigc\Infinite-Canvas\workflows\flux2-klein-t2i.json
 * 注入：提示词 -> 节点3(CLIPTextEncode).text；种子 -> 节点9(RandomNoise).noise_seed
 * 输出：节点13(SaveImage)
 */
const runFlux2KleinT2I = async (config: ImageConfig): Promise<string> => {
  const workflow = JSON.parse(JSON.stringify(WORKFLOW_FLUX2_KLEIN_T2I));
  workflow["3"].inputs.text = config.prompt || "";
  workflow["9"].inputs.noise_seed = generateSeed();
  const fileInfo = await submitAndPoll(workflow, "13", { interval: 2000, timeout: 300000 });
  return await downloadAsBase64(fileInfo);
};
...
const imageRequest = async (config: ImageConfig, model: ImageModel): Promise<string> => {
  switch (model.modelName) {
    ...
    case "flux2-klein-t2i":
      return await runFlux2KleinT2I(config);
    case "qwen-edit-2511":
