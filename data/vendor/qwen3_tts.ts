...
const vendor: VendorConfig = {
  ...
  description: "基于本地 ComfyUI 的 Qwen3 TTS 语音合成工作流。需要 ComfyUI 环境已配置 Qwen3-TTS 自定义节点。",
  inputs: [
    { key: "baseUrl", label: "ComfyUI 地址", type: "text", required: true, placeholder: "http://localhost:8188" },
    { key: "modelSize", label: "模型大小", type: "text", required: false, placeholder: "1.7B" },
  ],
...
const ttsRequest = async (config: TTSConfig, model: TTSModel): Promise<string> => {
  ...
  logger(`[Qwen3 TTS] 语音指令: "${voiceInstruct || "默认"}", 模型: ${modelSize}`);
