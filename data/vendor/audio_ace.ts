...
const vendor: VendorConfig = {
  ...
  description: "基于本地 ComfyUI 的 AceStep 1.5 XL Turbo 音乐/音频生成工作流。需要 ComfyUI 环境已配置 AceStep 模型及对应节点。",
  inputs: [
    { key: "baseUrl", label: "ComfyUI 地址", type: "text", required: true, placeholder: "http://localhost:8188" },
    { key: "defaultBpm", label: "默认 BPM", type: "text", required: false, placeholder: "95" },
    { key: "defaultKey", label: "默认调性", type: "text", required: false, placeholder: "E minor" },
  ],
  inputValues: { baseUrl: "http://localhost:8188", defaultBpm: "95", defaultKey: "E minor" },
  models: [{
...
const ttsRequest = async (config: TTSConfig, model: TTSModel): Promise<string> => {
  ...
  // 3. 设置 BPM（从 inputValues 或默认值）
  const bpm = parseInt(vendor.inputValues.defaultBpm || "95", 10);
  workflow["94"]["inputs"]["bpm"] = isNaN(bpm) ? 95 : bpm;

  // 4. 设置调性
  workflow["94"]["inputs"]["keyscale"] = vendor.inputValues.defaultKey || "E minor";

  // 5. 设置时长（默认 30 秒，可由语音速率粗略调节）
  const duration = Math.max(15, Math.min(300, config.speechRate > 0 ? Math.round(config.speechRate) : 30));
  workflow["94"]["inputs"]["duration"] = duration;
  workflow["98"]["inputs"]["seconds"] = duration;

  logger(`[AceStep Audio] BPM: ${bpm}, 调性: ${workflow["94"]["inputs"]["keyscale"]}, 时长: ${duration}s`);
