...
5. **单视图静物摄影** — 衍生状态仅需一张静物照。与原prop design图同机位、同光线、同背景——仅有道具的状态发生了变化
...
> 真人都市道具衍生仅需**单张**静物摄影。同一机位、同一光线、同一背景——仅有道具状态发生了变化。不是2x2 grid、不是多角度、不是turnaround。

### 全部状态变体共用此规范

| 项目 | 摄影要求 |
|---|---|
| 背景 | 素灰无缝背景纸 #B0B0B0（哑光），或浅木纹桌面/米白台面——与原prop design图**完全一致** |
| 光线 | 与原prop design图**完全一致**的窗光或柔光箱设置。光位、光比、色温不变 |
| 角度 | 与原prop design图**完全一致**的机位和角度。确保是"同一件物品在不同时间的同一位置被拍" |
...
基于原prop design图的同一机位和光线条件，{prop type}，{material description}，
...
human realistic摄影画质、35mm full-frame photography
...
| R3 | 必须是**单张**静物摄影——单视图、非2×22x2 grid、非多角度、非turnaround |
| R4 | 必须与原prop design图保持同一机位、同一光线、同一背景——仅道具状态不同 |
