...
| 项目 | 约束 |
|---|---|
| 五官 | 由角色描述自然推导，根据角色性格/身份生成差异化脸型与轮廓（如方/圆/尖/瓜子/长脸等），整体保持黏土圆润感（无尖锐棱角） |
...
| 造型 | 按角色身份推导：童话角色→简单黏土发饰（小花/布条/木簪）；平民→自然发束或简单扎发 | 自然发束、简单束发 |
...
> 角色服装应严格依据「角色描述」中的服饰描写生成。若角色描述中已指定具体服饰（如龙袍、铠甲、礼服等），则以此为准，在黏土风格下简化呈现。
>
> 角色描述未指定服饰时，女性默认为简化素色裙装，男性默认为简化素色上衣+裤装。

### 服装推导规则

| 情况 | 处理方式 |
|---|---|
| 角色描述中含具体服饰（如龙袍/铠甲/礼服等） | 直接使用描述中的服饰，保留款式/颜色特征，以黏土风格简化呈现 |
| 角色描述仅提示身份/气质（如国王/将军/仙女等） | 根据身份推导符合角色的正式服饰，黏土风格简化呈现 |
| 角色描述无任何服饰或身份线索 | 使用默认简化素色服装 |
...
{发色}{发长}，黏土发型，{基础造型}，简单黏土发饰（小花/布条/木簪/角色身份对应发饰），
{依据角色描述生成的服饰描述, 有具体服饰时以此为准（黏土风格简化呈现）, 无则默认为简化素色服装},
...
|---|---|
| R1 | 必须为「哑光黏土质感」状态 |
| R2 | 服装以角色描述中的服饰为准（黏土风格简化呈现）；角色描述未指定时，女性为简化素色裙装，男性为简化素色上衣+裤装 |
| R3 | 允许简单黏土风格发饰（小花/布条/木簪/角色身份对应发饰），禁止现代配饰 |
| R4 | 必须指定「纯净中性灰背景」 |
| R5 | 必须指定「四视图一致性」 |
...
| 编号 | 严禁 |
|---|---|
| X1 | 无理由使用与角色描述不符的基础服装（角色描述已指定服饰时不得降级为简化素色款） |
| X2 | 真人写实摄影质感/照片级真实 |
| X3 | 冷硬光线/硬阴影/高对比 |
| X4 | 尖锐棱角/攻击性姿态 |
| X5 | 复杂场景背景（必须纯灰底） |
